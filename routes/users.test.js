...
describe("POST /users/:username/jobs/:id", function () {
  test("works for same user", async function () {
    const jobRes = await db.query(`
      ...
      VALUES ('Apply Job', 100000, 0.5, 'c1')
      RETURNING id
    `);
  ...
  test("user get includes applied job", async function () {
    const jobRes = await db.query(`
      INSERT INTO jobs (title, salary, equity, company_handle)
      VALUES ('Applied Job', 100000, 0.5, 'c1')
      RETURNING id
    `);
    ...
    expect(resp.body.user.jobs).toEqual([jobId]);
  });
